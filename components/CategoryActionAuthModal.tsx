...
const CategoryActionAuthModal: React.FC<CategoryActionAuthModalProps> = ({ 
  ...
  const styles = actionType === 'edit'
    ? {
        iconWrap: 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400',
        inputFocus: 'focus:ring-amber-500',
        errorWrap: 'bg-amber-50 dark:bg-amber-900/20',
        errorIcon: 'text-amber-600 dark:text-amber-400',
        errorText: 'text-amber-700 dark:text-amber-300',
    ...
    : {
        ...
        errorWrap: 'bg-red-50 dark:bg-red-900/20',
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden border border-slate-200 dark:border-slate-700 p-6 relative">
        ...
        <div className="flex flex-col items-center mb-6">
          <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-4 ${styles.iconWrap}`}>
            <Lock size={28} />
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <input
              ...
              className={`w-full p-3 rounded-xl border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white focus:ring-2 ${styles.inputFocus} outline-none transition-all text-center tracking-widest`}
              placeholder="请输入密码"
              autoFocus
          ...
          {error && (
            <div className={`p-2 rounded-lg flex items-center gap-2 ${styles.errorWrap}`}>
          ...
          <div className="flex gap-2">
            ...
            <button
              type="submit"
              className={`flex-1 px-4 py-2.5 text-white rounded-xl transition-colors font-medium disabled:opacity-50 ${styles.submit}`}
              disabled={isVerifying}
            >
